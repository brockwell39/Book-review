...
def book_api(isbn):
    # for some reason the route is running with string= style2.css I think due to using string for isbn
    if isbn != "style2.css":
        ...
        average = db.execute("SELECT AVG(rating) from reviews WHERE book_id=:id",{"id":id}).fetchone()
        review_count = len(reviews)
        if review_count != 0:
            average_score = round(average[0],2)
